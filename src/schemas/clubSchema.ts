...
export default pgTable("clubs", {
  ...
  name: text("name").notNull(), // clubs might have the same name across organisations 
  description: text("description").notNull(), // this will need to be expanded, depending on what information we want to display
  membership_fee: numeric("membership_fee", { precision: 4, scale: 2 }).notNull(), // storing a money amount of up to 99.99
});